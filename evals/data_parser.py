...
def _find_data_dir(subfolder: str) -> str:
    base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    for candidate in ("DATA", "data"):
        path = os.path.join(base, candidate, subfolder)
        if os.path.exists(path):
            return path
    return os.path.join(base, "DATA", subfolder)


TRUE_DATA_DIR = _find_data_dir("true_data")
NOISY_DATA_DIR = _find_data_dir("noisy_data")
